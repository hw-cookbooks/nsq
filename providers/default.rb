...
NSQ_CONFIG_FILE_SUPPORT = %w(nsqadmin nsqd nsqlookupd).freeze
...
action :add do
  ...
  options = if new_resource.use_config_file
              "-config #{config}"
            else
              new_resource.config.map do |k, v|
                k = k.to_s.tr('_', '-')
                ["-#{k}", v.inspect].join('=')
              end.join(' ')
            end
...
action :remove do
  ...
  node.normal['nsq'][:enabled] = enabled
end
